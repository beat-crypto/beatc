...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the BEATCOIN and zBEATC money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 12777, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in BEATCOIN/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in BEATCOIN/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in BEATCOIN/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-backupzbeatcoin=<n>", strprintf(_("Enable automatic wallet backups triggered after each zBEATC minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizebeatcoinamount=<n>     " + strprintf(_("Keep N BEATCOIN anonymized (default: %u)"), 0) + "\n";
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZBEATCOINMinted();
                        RecalculateZBEATCOINSpent();
                    }
                    RecalculateBEATCOINSupply(1);
                }
...
//    if (nAnonymizeBeatCOINAmount < 2) nAnonymizeBeatCOINAmount = 2;
    ...
    /* Denominations
       ...
       For example:
       1BEATCOIN+1000 == (.1BEATCOIN+100)*10
       10BEATCOIN+10000 == (1BEATCOIN+1000)*10
    */
