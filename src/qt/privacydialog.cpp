                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zBEATC ought to be enough for anybody." - Bill Gates, 2017
    ui->zBEATCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzBEATCSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzBEATC->setEnabled(false);
        ui->pushButtonMintzBEATC->setToolTip(tr("zBEATC is currently disabled due to maintenance."));

        ui->pushButtonSpendzBEATC->setEnabled(false);
        ui->pushButtonSpendzBEATC->setToolTip(tr("zBEATC is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zBEATCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzBEATC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBEATC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zBEATC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zBEATC in ") +
...
void PrivacyDialog::on_pushButtonSpendzBEATC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zBEATC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zBEATC
        sendzBEATC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zBEATC
    sendzBEATC();
}

void PrivacyDialog::on_pushButtonZBeatCOINControl_clicked()
{
    ZBeatCOINControlDialog* zBEATCControl = new ZBeatCOINControlDialog(this);
    zBEATCControl->setModel(walletModel);
    zBEATCControl->exec();
}

void PrivacyDialog::setZBeatCOINControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBEATCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBEATC()
{
    ...
    double dAmount = ui->zBEATCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zBEATC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " BEATCOIN </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zBEATC</b>";
    ...
    // use mints from zBEATC selector if applicable
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBEATC Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zBEATC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zBEATC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zBEATC "));
    ui->labelzBEATCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzBEATCSyncStatus->setVisible(fShow);
}
